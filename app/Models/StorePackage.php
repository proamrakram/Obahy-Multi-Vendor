<?php

namespace App\Models;

use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;

class StorePackage extends Model
{
    use HasFactory;

    protected $fillable = [
        'package_name_en',
        'package_name_ar',
        'package_description_en',
        'package_description_ar',
        'package_price',
        'package_currency',
    ];

    public function items()
    {
        return $this->hasMany('App\Models\StorePackageItem', 'package_id');
    }

    public function scopeLanguage($query)
    {
        if (session('lang') == 'ar') {
            $lang = ['id', 'package_name_ar',  'package_description_ar', 'package_price', 'package_currency'];
        } else {
            $lang = ['id', 'package_name_en', 'package_description_en', 'package_price', 'package_currency'];
        }

        return $query->select($lang);
    }

    public function currency()
    {
        return $this->belongsTo('App\Models\Currency', 'package_currency');
    }
}
