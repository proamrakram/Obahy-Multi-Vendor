...
class CustomersController extends Controller
{
    ...
    {
        $customers = User::where('is_delete', 0)->where('active', 1)->where('user_type', 'customer')->orderBy('created_at', 'desc')->paginate(10);
        $countries = Country::where('status', 'active')->where('is_delete', 0)->get();
        $customer_count =  User::where('is_delete', 0)->where('active', 1)->where('user_type', 'customer')->count();
